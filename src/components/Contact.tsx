...
const SHOOT_TYPES = ['Kids Photography', 'Maternity Photography', 'Wedding Photography', 'Event Photography', 'Other']
...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    openWhatsApp(bookingMessage({ name: form.name, phone: form.phone.trim(), shootType: form.shootType, message: form.message }))
    fetch('/api/contact', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, phone: form.phone.trim() }) }).catch(() => {})
    setSuccess(true)
    setForm(initialForm)
    setLoading(false)
  }

  const inputCls = (field: string) => `w-full bg-white border rounded-lg ${fieldErrors[field] ? 'border-red-400' : 'border-cream-3 focus:border-terra'} text-warm-brown text-sm px-4 py-3 outline-none transition-colors duration-200 placeholder:text-warm-muted`
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16">
        ...
        <div>
          ...
          <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl font-light text-warm-brown leading-tight mb-6">
            Let&apos;s Create<br />Something<br /><em className="italic">Beautiful Together</em>
          </h2>
          <p className="text-warm-mid font-light leading-relaxed mb-10">
            Ready to book? Fill in the form and Sandy will get back to you within 24 hours with availability and pricing.
          </p>
          ...
          <div className="space-y-5 mb-10">
            <div className="flex items-center gap-4 text-warm-mid">
              ...
              <a href="mailto:sandy@sandyphotography.in" className="text-sm hover:text-terra transition-colors duration-200">sandy@sandyphotography.in</a>
            </div>
            <div className="flex items-center gap-4 text-warm-mid">
              <FiPhone className="text-terra flex-shrink-0" size={16} />
              <a href="tel:+918099865977" className="text-sm hover:text-terra transition-colors duration-200">+91 8099865977</a>
            </div>
            <div className="flex items-center gap-4 text-warm-mid">
          ...
          <div className="flex items-center gap-4">
            <a href="https://instagram.com" target="_blank" rel="noopener noreferrer"
              className="w-10 h-10 flex items-center justify-center border border-cream-3 rounded-full text-warm-mid hover:border-terra hover:text-terra transition-all duration-300" aria-label="Instagram">
              <FiInstagram size={16} />
            </a>
            <a href="https://facebook.com" target="_blank" rel="noopener noreferrer"
              className="w-10 h-10 flex items-center justify-center border border-cream-3 rounded-full text-warm-mid hover:border-terra hover:text-terra transition-all duration-300" aria-label="Facebook">
              <FiFacebook size={16} />
            </a>
          </div>
        </div>
          ...
          {success ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-16">
              ...
              <a href="https://wa.me/918099865977" target="_blank" rel="noopener noreferrer"
                ...
                <FaWhatsapp size={16} /> Or message on WhatsApp
              </a>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              <div>
                ...
                <input type="text" name="name" value={form.name} onChange={handleChange} placeholder="e.g. Priya Sharma" className={inputCls('name')} />
              ...
              <div>
                ...
                <div className="flex">
                  ...
                  <input type="tel" name="phone" value={form.phone} onChange={handleChange} placeholder="10-digit number" maxLength={10}
              ...
              <div>
                ...
                <textarea name="message" value={form.message} onChange={handleChange}
                  placeholder="Tell us about your vision, your baby's age, preferred dates..." rows={5}
                  className={`${inputCls('message')} resize-none`} />
                {fieldErrors.message && <p className="text-red-400 text-xs mt-1">{fieldErrors.message}</p>}
              </div>
              {error && <div className="border border-red-400/30 bg-red-50 text-red-500 text-sm px-4 py-3 rounded-lg">{error}</div>}
